#!/bin/bash
#
# mdtest 单目录并发元数据测试（MPI 版）
#
# 与 mdtest_multi.sh 的区别：WIDTH=1（不按 rank 分叉，所有 rank 在同一棵树下），
# 不加 -u（不为每个 task 单独建目录）。用于测单目录下的并发元数据压力。
#
...
#   3. --mca btl_tcp_if_include 指定的网卡名/网段在所有节点都存在。

set -eu

nodes_file=./nodeslist
TARGET=/mnt/yrtest/zl/fio/dir1/dir2/dir3/dir4/dir5/dir6/dir7/mdtest

# 用于 MPI 进程间通信的网卡：可填接口名（如 eth0 / bond4）或网段（CIDR）。
# IFACE=10.16.17.0/24
IFACE=eth0

DEPTH=1          # 树深度
WIDTH=1          # 分支数（单目录测试固定为 1）
num_files=100000
num_procs=720
#num_procs=640

# 每个叶子目录里的文件数。
# 总文件数 = num_procs * WIDTH * files_per_dir，WIDTH=1 时即 num_procs * files_per_dir
...
#   --mca btl_tcp_if_include      指定 TCP 网卡/网段（白名单）
